...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        #self.img_files = self.img_files[:1000]
        ...
        #                    self.img_files]
        self.label_files = [x[:-3].replace("JPEGImages", "Annotations") + "xml" for x in self.img_files]

        # Check cache
        cache_path = '/datav/wish/yolov5/runs/bigblue_{}.cache'.format(get_md5(path))  # cached labels
        if os.path.isfile(cache_path):
            cache = torch.load(cache_path)  # load
            #if cache['hash'] != get_hash(self.label_files + self.img_files):  # dataset changed
            #    #cache = self.cache_labels(cache_path)  # re-cache
            #    cache = self.cache_labels_multi_thread(cache_path)
        else:
        ...
        for i, file in enumerate(pbar):
            ...
            if l.shape[0]:
                ...
                assert (l >= 0).all(), 'negative labels: {}, {}'.format(file, l)
                assert (l[:, 1:] <= 1).all(), 'non-normalized or out of bounds coordinate labels: {}, {}'.format(file, l[:, 1:])
    ...
    def load_objs(self, file, width, height):
        ...
        label_map = ['vehicle', 'tyre', 'chepai', 'pot', 'tank']
        #label_map = ['vehicle', 'tyre', 'chepai']
        objs = []
        ...
        while object_ is not None:
            ...
            #print(width, height, (xmin, ymin, xmax, ymax), (x, y, w, h))

            if name not in label_map:
                #assert name != "tank" and name != "pot"
                # if name == "pot":
                #     continue
    ...
    def load_objs_bigblue(self, file, width, height):
        with open(file, "r") as f:
            data = f.read()
    ...
    def load_label_thread(self, fs, tid):
        
        x = {}
        total = len(fs)
        lastp = -1
        for index in range(total):
            img_file, ann_file = fs[index]
            ...
            if os.path.isfile(ann_file):
                ...
                #l = self.load_objs(label, *shape)
                l = self.load_objs_bigblue(ann_file, *shape)
            if len(l) == 0:
                l = np.zeros((0, 5), dtype=np.float32)
            x[img_file] = [l, shape]

            if index % 100 == 0:
                precent = int((index + 1) / total * 100)
                if precent != lastp:
                    lastp = precent
                    print("{}. {} / {}, {} %".format(tid, index + 1, total, precent))
        
        self.update_multi_result(x)


    def update_multi_result(self, values):

        with self.lock:
            self.result.update(values)


    def cache_labels_multi_thread(self, path='labels.cache'):
        ...
        nworker = 256
        step = math.ceil(total / nworker)
        nblock = 0
        for i in range(0, total, step):
            begin = i
            end = min(i + step, total)
            t = threading.Thread(target=self.load_label_thread, args=(list(zip(self.img_files[begin:end], self.label_files[begin:end])), nblock))    
            t.start()
            ts.append(t)
            nblock += 1
    ...
    def cache_labels(self, path='labels.cache'):
        ...
        for (img, label) in pbar:
            ...
            if os.path.isfile(label):
                ...
                #    #l = np.array([x.split() for x in f.read().splitlines()], dtype=np.float32)  # labels
                #l = self.load_objs(label, *shape)
                l = self.load_objs_bigblue(label, *shape)
            if len(l) == 0:
                l = np.zeros((0, 5), dtype=np.float32)
            x[img] = [l, shape]
            # except Exception as e:
